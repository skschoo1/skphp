...
define('STYLE_PATH',     RUNTIME_PATH.'style/'); 	// 应用样式缓存目录
...
final class Application {
	...
	static public function init() {
		require SK_PATH.'/library/sk/jsmin.class.php';
		require SK_PATH.'/library/sk/cssmin.class.php';
	...
	static public function autoload() {
		...
		self::$_lib = array(
			...
			'mysql'    	 =>      LIB_PATH.'sk/mysql.class.php',
		...
		foreach (self::$_lib as $key => $value){
			...
			if($lib == 'Mysql')
			{
				self::$_instance[$key] =  $lib::getInstance();
			}else{
	 ...
	 * @paramstring 标识
	 * @paramstring 类名
	 * @returnSingleton返回应用中的唯一对象实例
	 */
	public static function getInstance($identity,$className) {
		if (!(self::$_instance[$identity] instanceof $className))
		{
			self::$_instance[$identity] = new $className();
		}
